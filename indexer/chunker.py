...
def load_language(language: str):
    ...
    raise AttributeError(f"Could not find a Language object in package {module_name}.")
...
def build_chunks_for_node(
    node, source_bytes: bytes, file_path: str, language: str
) -> list[dict]:
    ...
    def make_chunk(
        span_start: int, span_end: int, start_token_index: int, end_token_index: int
    ):
        chunk_text = node_text[span_start:span_end]
        chunk_start_byte = node.start_byte + byte_offset_from_char_index(
            node_text, span_start
        )
        chunk_end_byte = node.start_byte + byte_offset_from_char_index(
            node_text, span_end
        )
        return {
            "file": str(file_path),
...
def chunk_source(file_path: str, language: str) -> list[dict]:
    ...
    for method_node in find_method_nodes(root, language):
        chunks.extend(
            build_chunks_for_node(method_node, source_bytes, file_path, language)
        )

    return merge_small_chunks(chunks)


def chunk_code_from_string(
    code: str, language: str, file_path: str = "<unknown>"
) -> list[dict]:
