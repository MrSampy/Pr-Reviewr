...
def _prepare_code_chunks(repo_path: str) -> List[dict]:
    ...
    for file_data in code_files:
        ...
        if file_path.exists():
            ...
            chunks = chunker.chunk_code_from_string(file_data['content'], language, file_path=file_data['path'])
...
def index_repository(
    ...
    if not all_items:
        raise RuntimeError('No indexable items found. Check repository path and ADO configuration.')
...
def index_repository_to_chroma_from_env(repo_path: str = '.') -> None:
    ...
    index_repository(repo_path=repo_path, collection_name=collection_name, persist_directory=persist_directory)
