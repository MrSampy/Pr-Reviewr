...
def get_ado_config():
    organization = _get_env_value('ADO_ORGANIZATION', 'AZURE_DEVOPS_ORGANIZATION', 'ADO_ORG', 'AZURE_DEVOPS_ORG')
    ...
    if not pat:
        raise EnvironmentError('ADO_PAT or AZURE_DEVOPS_EXT_PAT environment variable is required')
    ...
    if org_url:
        ...
        raise EnvironmentError('ADO_ORGANIZATION or ADO_ORG_URL environment variable is required')
...
def extract_line_from_context(thread_context: dict) -> int | None:
    ...
    for key in ('rightFileStart', 'leftFileStart', 'rightStartLine', 'leftStartLine', 'line'):
...
def get_pr_threads(base_url: str, project: str | None, repository_id: str, pr_id: int, headers: dict) -> list[dict]:
    project_path = f'{project}/' if project else ''
...
def extract_pr_comments_from_ado(repo_path: str = '.', min_pr_id: int = 0) -> list[dict]:
    ...
    for pr in get_closed_prs(base_url, project, headers):
        ...
        for thread in get_pr_threads(base_url, project, repository_id, pr_id, headers):
            ...
            for comment in thread.get('comments', []):
                ...
                display_name = comment_author.get('displayName') or comment_author.get('uniqueName')
                if display_name:
