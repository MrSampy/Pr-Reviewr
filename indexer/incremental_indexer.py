...
def reindex_files(changed_files: List[str], repo_path: str, collection):
    for file in changed_files:
        ...
        ids = [f"code|{normalized_file}|{item['chunk_index']}|{idx}" for idx, item in enumerate(items)]
        metadatas = [
            {
                'file': item['file'],
...
def update_pr_comments(collection):
    ...
    embeddings = embedder.embed_chunks(items)
    ids = [f"pr_comment|{item['file']}|{item['chunk_index']}|{idx}" for idx, item in enumerate(items)]
    metadatas = [
        {
            'file': item['file'],
            'author': item.get('author') or '',
